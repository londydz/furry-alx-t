...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:pass@localhost:5432/booking'
...
class Venue(db.Model):
    ...
    seeking_description = db.Column(db.ARRAY(db.String))

    shows = db.relationship('Show', backref='venues', lazy=False)

    def __repr__(self):
        return f'<Venue ID: {self.id}, Name: {self.name}, City: {self.city}, State: {self.state}, Address: {self.address}, Phone: {self.phone}, Genres: {self.genres},Image Link: {self.image_link}, Facebook Link {self.facebook_link}, website Link: {self.website_link},Seeking Venue: {self.seeking_talent}, Seeking Description: {self.see}>'
...
class Artist(db.Model):
    ...
    def __repr__(self):
        return f'<Artist: {self.id}, Name: {self.name},City: {self.city}, State: {self.state}, Phone: {self.phone},Genres: {self.genres}, Image Link: {self.image_link},Facebook Link: {self.facebook_link}, Website Link:{self.website_link}>'
...
class Show(db.Model):
    ...
    def __repr__(self):
        return f'<Show {self.id},Artist ID: {self.artist_id}, Venue ID: {self.venue_id}, Start time: {self.date}>'
...
def venues():
 ...
 # num_shows should be aggregated based on number of upcoming shows per venue.
    venue_areas = db.session.query(Venue.city, Venue.state).group_by(
...
def delete_venue(venue_id):
    ...
    try:
        ...
        db.session.commit()
        flash('Venue ' + venueName + ' was  deleted successfully!')
    except:
        db.session.rollback()
...
def show_artist(artist_id):
    ...
    curr_shows = []
    for shw in past_shows:
        shw = {}
        shw["venue_name"] = show.venues.name
        shw["venue_id"] = show.venues.id
        shw["venue_image_link"] = show.venues.image_link
        shw["start_time"] = show.start_time.strftime("%m/%d/%Y, %H:%M:%S")

        curr_shows.append(shw)
...
def edit_artist(artist_id):
 ...
 # Returns object based on primary key, or None.  Guessing get is faster than filter_by
    ...
    artist = Artist.query.get(artist_id)
    # convert genre string back to array
    form.genre.data = artist.genre.split(",")
...
def edit_venue(venue_id):
    ...
    if not venue:
        ...
        genres = [genres.name for genre in venue.genres]
...
def create_artist_submission():  # called upon submitting the new artist listing form
    form = ArtistForm()
...
def shows():
    ...
    for show in shows:
        ...
        shw["venue_id"] = show.venues.id
        shw["venue_name"] = show.venues.name
        shw["artist_id"] = show.artists.id
        shw["artist_name"] = show.artists.name
        shw["artist_image_link"] = show.artists.image_link
        shw["start_time"] = show.start_time.strftime("%m/%d/%Y, %H:%M:%S")
